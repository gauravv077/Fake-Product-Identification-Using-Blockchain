...
public class updateorder extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            ResultSet rs = st.executeQuery("select email from orderformurchats where orderid='" + orderid + "'");
            if (rs.next()) {
                mail = rs.getString(1);
            }
            String[] email = {mail};
            
            String body = "Welcome,\nOrder id '" + orderid + "' is received to user.";

            if (status.equals("Rejected")) {
                body = "\nOrder id '" + orderid + "' is rejected by user.";
            }
            MailUtil msg = new MailUtil();
            msg.sendMail(email, email, "Order Received.", body);
