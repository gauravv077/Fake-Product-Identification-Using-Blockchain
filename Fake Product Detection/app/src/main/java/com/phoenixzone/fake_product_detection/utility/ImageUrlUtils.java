...
public class ImageUrlUtils {
    ...
    public static ArrayList<CartItem> productList = new ArrayList<CartItem>();
    ...
    public boolean checkProductExist(String product) {
        for (int i = 0; i < this.productList.size(); i++) {
            CartItem cartItem = productList.get(i);
            if (cartItem.getProduct_name().equals(product)) {
                int count = cartItem.getProduct_count();
                count++;
                cartItem.setProduct_count(count);
    ...
    public void addProductList(CartItem productInfo) {
